...
while :; do
  ...
  st=$(printf '%s' "$resp" | MU="$MSG_UID" node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{let j;try{j=JSON.parse(s)}catch(e){process.exit(2)}const a=Array.isArray(j)?j:(j.messages||j.data||[]);const m=a.find(x=>x&&x.uid===process.env.MU);process.stdout.write(m&&m.status?m.status:"")})') \
