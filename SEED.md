...
- `https://github.com/plow-pbc/seed-hermes-plow` — installs the `plow_chat` gateway (its activation lands `PLOW_CHAT_*` into the scaffold's `data/.env`) and the `plow-connectors` skill the producers read Gmail / Google Calendar / Slack through **into a pre-existing seed-hermes scaffold**. It does NOT create the scaffold (no `compose.yaml`, no Hermes image pull): the base scaffold — a host `compose.yaml`, `./data:/opt/data`, and the container **running** — is a HARD PRECONDITION the operator/host must already have (the [`### Requirements`](#requirements) hardware row), not something this dep provides. That pre-existing Docker Hermes scaffold IS the producer runtime — the agent half installs into it; there is no Plow desktop app and no local agent daemon in this graph.
...
# executed — Verification step 7 instead requires every card to differ from
...
3. Scaffold container is **exec-able**, not merely running: the agent-half cron registration and the activation block both `docker compose exec` into the seed-hermes scaffold, so a container that is up but cannot start new processes (the canonical case: an arm64 scaffold image on an x86_64 host whose qemu binfmt handler was deregistered after boot — the long-running main process survives, but every `exec` returns `exec format error`) would pass a naive "running" check and then fail every cron/activation step mid-install. Prove exec works up front with a fast no-op exec — fail-fast with an actionable message naming the qemu-binfmt cause and remedy when the container is up but exec fails:
    ...
    EXEC_ERR=$(docker compose -f "$HERMES_SCAFFOLD/compose.yaml" exec -T "$HERMES_SERVICE" true 2>&1) \
      || { echo "FAIL: seed-hermes scaffold container is not running or not exec-able (docker: ${EXEC_ERR}). If it is up but the scaffold image is arm64 running under qemu on an x86_64 host, the qemu binfmt handler was likely deregistered after boot — re-register it with: docker run --privileged --rm tonistiigi/binfmt --install all" >&2; exit 1; }
    ```
...
The install is NOT complete when the skills are merely on disk — it is complete when each dashboard producer has **executed** once in the Hermes runtime and the cards exist. This action is the root's single Phase-2 step (the block in [`## Dependencies`](#dependencies)); [Verification](#verification) step 7 asserts its observable outcome.
...
3. The installer does not parse the cron run's output — the Pi store is the ground truth, asserted by Verification step 7. A populated store proves the whole producer chain: current skill code visible to the container, endpoint reachable *from inside the container* (not just from the Docker host), valid token, store writable.
