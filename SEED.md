...
| auth     | ChatGPT account for Hermes' `openai-codex` OAuth | in-flow   | the one-time device-code approval driven by `seed-hermes`'s `auth-openai-codex.sh` during its recursion (lands `data/auth.json`); see [the Pi-side hands-on moments](#environment-is-probed-up-front) | |
...
3. The Pi-side hands-on moments (can't be scripted): **ChatGPT `openai-codex` OAuth** for the Hermes scaffold (`seed-hermes` opens a device-code page during its recursion — the operator approves it in a browser; lands `data/auth.json`), and the **Plow Chat phone-bind** activation (`seed-hermes-plow`, unless `PLOW_CHAT_TOKEN` is set). Both are in-flow `auth` rows, surfaced when their dep recurses; neither is collectible at prepare time (the device code is minted live).
