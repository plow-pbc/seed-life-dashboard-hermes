...
| system   | Docker + Compose v2 on the Pi (the seed-hermes scaffold's runtime) | preflight | pre-installed on the Pi; arm64 is native — `nousresearch/hermes-agent:latest` ships a `linux/arm64` variant, so the Pi runs it without qemu and the agent half can `docker compose exec` into it. With passwordless sudo the `seed-hermes` dep's host-tools check installs the Compose plugin if absent; without it, the install is **folded into the same prepare-script `ssh -t … sudo …` run** as the package line above. On Raspberry Pi OS (Debian 13 trixie) `docker compose` (Compose v2) is **not** apt-installable by default (`docker-compose-plugin`/`docker-compose-v2` have no candidate), so the folded line installs Compose v2 **explicitly** — add Docker's official apt repo, or fetch the `docker-compose` cli-plugin binary — engine-present ≠ Compose-present | |
