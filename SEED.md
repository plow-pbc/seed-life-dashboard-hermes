...
- `https://github.com/plow-pbc/seed-hermes-plow` — installs the `plow_chat` gateway (its activation lands `PLOW_CHAT_*` into the scaffold's `data/.env`) and the `plow-connectors` skill the producers read Gmail / Google Calendar / Slack through **into a pre-existing seed-hermes scaffold**. It does NOT create the scaffold (no `compose.yaml`, no Hermes image pull): the base scaffold — a host `compose.yaml`, `./data:/opt/data`, and the container **up and exec-able** — is a HARD PRECONDITION the operator/host must already have (the [`### Requirements`](#requirements) hardware row), not something this dep provides. That pre-existing Docker Hermes scaffold IS the producer runtime — the agent half installs into it; there is no Plow desktop app and no local agent daemon in this graph.
