...
- `https://github.com/plow-pbc/seed-durable-ssh` — listed **first** so SSH reachability and key auth to the Pi are proven (or fail loudly) before anything else installs, and every later SSH hop — the on-Pi scaffold install, the agent recursion's `docker compose exec`, viewer blocks, verification — rides its ControlMaster multiplexing (one authenticated connection for the whole install). Its `SEED_SSH_TARGET` input is derived, never re-collected: `SEED_SSH_TARGET=$LD_PI_SSH_TARGET`. When key auth is missing, its `ssh-copy-id` step contributes a Pi-password prompt to the prepare-script; when the Pi also lacks passwordless sudo and packages/Compose are missing, the `ssh -t … sudo …` package fold (see [environment is probed up front](#environment-is-probed-up-front)) prompts for that same Pi password too — both up front in the operator's shell, never mid-install.
