...
The umbrella collects three operator inputs once at install time (tier-3; the secret calendar URL is collected off-transcript via the prepare-script, the non-secret Pi target + display name via agent questions; persisted only in the mode-600 inputs file `~/.config/seed/seed-life-dashboard-hermes.env`, outside the repo):
...
- **A Raspberry Pi** reachable over SSH from the host you run the install on — this is the **single machine** for the whole stack: it runs the Hermes scaffold container (the producer agent) AND the native kiosk viewer. The base scaffold is **not** a precondition you build yourself — [`seed-hermes`](https://github.com/plow-pbc/seed-hermes) (the umbrella runs it for you, first) creates `compose.yaml`, pulls the Hermes image, and starts the container on the Pi. The Pi needs Docker + Compose v2 and its viewer system packages (`node` ≥ 20.6, `npm`, `git`, Chromium, the emoji font). The installer probes for these up front: on a Pi with passwordless sudo it installs the missing ones over SSH unattended; otherwise the install is **folded into the prepare-script** (the same one-time operator run as the `ssh-copy-id` below) over `ssh -t … sudo …` — you enter the Pi's password up front for that `sudo` (and for `ssh-copy-id` if key auth is missing), never a separate mid-install script. Compose v2 is installed **explicitly** (Debian 13 _trixie_ has no apt package for it — Docker's official apt repo or the cli-plugin binary), not assumed from `apt`. The Hermes image is native `linux/arm64`, so it runs on the Pi without qemu. Key auth doesn't need to exist yet: if a probe finds none, the prepare-script runs `ssh-copy-id` first (and the package fold runs after it, so SSH already works).
...
Most of the install is automated. The agent asks the **two non-secret inputs** (the Pi SSH target and your display name) conversationally up front; the few **hands-on** steps are pinned to **boundaries** — up front in the prepare-script, or (only when the install host has no reusable Codex credential) once at the very end — never scattered mid-install:
