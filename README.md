...
- **A ChatGPT account** — the Hermes scaffold authenticates its model through ChatGPT's `openai-codex` OAuth device-code flow. This is the install's single end-of-install browser approval (deferred there because the device code is minted live against the running container); you approve it once (see [The hands-on moments](#the-hands-on-moments)).
